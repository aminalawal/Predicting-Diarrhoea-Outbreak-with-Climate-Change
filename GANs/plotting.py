    ...
    if data == 'mnist':
        ...
        if multivariate_mnist:
           ...
           save_mnist_plot_sample(samps, epoch, identifier, n_samples=6, labels=labs)
            #save_mnist_plot_sample(samps.reshape(3736,seq_length,9), epoch, identifier, n_samples=6, labels=labs)
    elif 'eICU' in data:
...
def save_plot_sample_Original(samples, idx, identifier, n_samples=6, num_epochs=None, ncol=2):
    print("I came tho this FXN SAVE PLOT SAMPLE")
...
def plot_sine_evaluation(real_samples, fake_samples, idx, identifier):
    ...
    """
    ### frequency
    print("I came tho this FXN PLOT SINE EVALUATION")
...
def save_samples(vis_sample, identifier, epoch):
    print("I came tho this FXN SAVE SAMPLE")

    np.save('./experiments/plots/gs' + identifier + '_gs_epoch' + str(epoch).zfill(4) + '.npy', vis_sample)

    GenSamples=vis_sample
    np.save('./experiments/plots/GenSamples.npy',GenSamples)
    return True
...
def save_mnist_plot_sample(samples, idx, identifier, n_samples, labels=None):
    ...
    for m in range(nrow):
        # first column
        sample = samples[m, :, 0]
        axarr[m, 0].imshow(sample.reshape([img_size,1]), cmap='gray')
        axarr[m, 0].set_title(str(label_titles[m]))
        # second column
        sample = samples[nrow + m, :, 0]
        axarr[m, 1].imshow(sample.reshape([img_size,1]), cmap='gray')
...
def save_plot_sample(samples, idx=0, identifier=None, n_samples=9, num_epochs=None, ncol=3):
    print("I came tho this ALTERED FXN SAVE PLOT SAMPLE")
    #print(samples.shape[0])
    print(samples.shape)
    variable_names=['Loperamide', 'Evaporation', 'Precipation', 'Windspeed', 'Humidity', 'MaxTemp', 'AirTemp','Pressure', 'MinTemp']
    #assert n_samples <= samples.shape[0]
    assert n_samples % ncol == 0
    sample_length = samples.shape[1]
    ...
    nrow = int(9/ncol)
    #fig, axarr = plt.subplots(len(variable_names), 1, sharex=True, figsize=(6.5, 9))
    fig, axarr = plt.subplots(nrow, ncol, sharex=True, figsize=(15, 12))
    print("---------------------nrow and ncol ----------------------------")
    print(nrow,ncol)
    i=0
    for m in range(nrow):
        for n in range(ncol):
            # first column
            sample = samples[0, :,n*nrow + m]
            
            axarr[m, n].plot(x_points, sample, "blue")
            #axarr[m, n].plot(x_points, sample, "blue", sample2, "gold", sample3, "r")
            axarr[m, n].set_ylabel(variable_names[i])
            #axarr[m, n].plot(x_points, sample, "deepskyblue", sample2, "g", sample3, "b", sample4, "m", sample5, "orange", sample6, "r", sample7, "brown", sample8, "yellow", sample9, "maroon" )
            #axarr[m, n].plot(x_points, sample2, color=col)
            axarr[m, n].set_ylim(-1, 1)
            i=i+1
            #ax.legend('Lope', 'Wind', 'AirT', 'Evap', 'Humid', 'Press', 'Precip', 'Tmax', 'Tmin'),fontsize=17, loc='center',bbox_to_anchor=(0.55, 0.79)
    for n in range(ncol):
        axarr[-1, n].xaxis.set_ticks(range(0, sample_length, int(sample_length/4)))
    fig.suptitle(idx)
    fig.subplots_adjust(hspace = 0.15)
