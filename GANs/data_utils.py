...
def mnist(randomize=False):
    ...
    try:
        ...
        numerator = train - np.min(train, 0)
        denominator = np.max(train, 0) - np.min(train, 0)
        norm_data = numerator / (denominator + 1e-7)

        np.save('./data/mnist_train.npy', norm_data)
        ...
        for i in range(0, len(samples) - seq_len):
            _x = samples[i:i + seq_len]
            temp_data.append(_x)
        samples=np.array(temp_data)
...
def sine_wave(seq_length=2, num_samples=3758, num_signals=9,
        freq_low=1, freq_high=5, amplitude_low = 0.1, amplitude_high=0.9, **kwargs):
    """ Load and serialise """
    try:
        train = np.load('./data/test.npy')
        print('Loaded test from .npy')
    except IOError:
        print('Failed to load test data from .npy, loading from csv')
        # read from the csv
        train = np.loadtxt(open('./data/test.csv', 'r'), delimiter=',')
        train=train.reshape(len(train), num_signals)
        
        #Normalizing to (-1,1)
        scaler = MinMaxScaler(feature_range=(-1, 1))
        norm_data = scaler.fit_transform(train)
        
        np.save('./data/test1.npy', norm_data)
